...
| RPL | Requested privilege level, two bits in the selector; joins CPL and DPL in the access rules. |
...
### What Linux does inside a syscall (plain English)
...
On 64-bit x86-64 Linux, paging (MMU and page tables) carries most of the user vs kernel memory separation you rely on day to day. Segments are mostly a flat story; RPL still exists in the architecture, but the lesson is the same: hardware enforces some walls, the kernel enforces identity and policy, and exploits often aim at the kernel half of that stack.

## Minimal OS as a lab for yourself

You do not have to build an OS to understand the split. A tiny kernel, a GDT, and deliberate wrong selector values made the CPU fault in ways a PDF alone did not. If you try it, expect long nights and small wins. It pays off in intuition.
...
4. Real mode shows what happens when hardware enforcement is thin; protected mode and paging add layers.
